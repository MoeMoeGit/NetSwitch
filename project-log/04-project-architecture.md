...
- **选择**：优先通过 `route print 0.0.0.0` 读取默认路由接口 IP，PowerShell 作为对象查询和兜底。
...
- 网络状态检测使用 `_StatusCheckWorker(QThread)`，由启动、打开主窗口、托盘菜单、切换完成、方案保存/删除等关键动作触发。
- 开机恢复上次方案使用 `_RestoreProfileWorker(QThread)`，避免启动阶段同步执行网络切换阻塞界面。
